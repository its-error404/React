...
const BlogLogic = ({ Blogs, stitle,setBlogs }) => {
    ...
    return (
        <BlogContent className='pt-4 ml-4'>
            ...
            <h1 className='text-2xl font-extrabold text-black font-extra font-josefin-italic-500'>{stitle}</h1>
            {
                Blogs.map((blog) =>
                    (
                        <BlogDisplay key={blog.postID} className='box-content items-center w-7/12 px-5 py-4 mt-4 text-center border-4 border-red-600 border-double shadow border-spashadow-2xl'>
                        ...
                        <button className='px-2 py-2 mt-4 font-bold text-white bg-yellow-500 rounded shadow-lg hover:bg-rounded-sm'onClick={()=>setBlogs(blog.postID)}>Delete the Blog</button>
                         </BlogDisplay>
                    )
                        )
            }
